...
public class WorkforceRepositoryImpl extends BaseRepositoryImpl<Workforce, UUID> implements WorkforceRepository {
    ...
    public Workforce create ( Workforce workforce ) {
        String query = "WITH inserted_workforce AS ( " +
                "    INSERT INTO workforces (name, price_per_hour, working_hours, tva, productivity_factor, project_id) " +
                "    VALUES (?, ?, ?, ?, ?, ?) " +
                "    RETURNING * " +
                ") " +
                "SELECT iw.*, p.*, c.* " +
                "FROM inserted_workforce iw " +
                "JOIN projects p ON iw.project_id = p.id " +
